"""中文：定义翼型 metadata 查询与插入相关的 FastAPI 路由。
English: Defines FastAPI routes for querying and inserting airfoil metadata.
"""
...
def get_full_catalog(conn=Depends(get_connection), reynolds_number: float = 1e6) -> FileCatalogListResponse:
...
def get_geometry_files_with_file_names(request: GeometryFilesRequest, conn=Depends(get_connection)) -> dict[str, Any]:
    ...
    for file_name, path in paths.items():
        path = Path(path)
        if not path.is_file():
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Geometry file for '{file_name}' not found.")
