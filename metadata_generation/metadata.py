"""Metadata models and JSON persistence helpers for airfoil data."""
...
class AerodynamicMetadata:
    ...
    cl_alpha: Optional[float] = None  # Lift curve slope
...
class FoilMetadata:
    # ------------ General metadata ------------
    file_name: str                                                                  # Original file name (e.g. "naca2412.dat")  
    path: str                                                                       # Relative path to the .dat file
    family_series: Optional[str] = None                                             # Airfoil family or series (e.g. "NACA 4-digit", "Selig", etc.)        
    source: str = "uiuc_database"                                                   # Source of the data 
    
    # May include multiple flags, e.g. ["low_point_count", "open_curve"]
    quality_flags: list[str] = field(default_factory=list)                          # List of quality flags or issues identified during processing

    is_modified: bool = False                                                       # If the foil is modified version 
    is_smooth: bool = False                                                         # If the foil is smoothed version
    is_naca: bool = False                                                           # If the foil is a NACA airfoil
    naca_code: Optional[str] = None                                                 # NACA code if applicable
    ...
    # ------------ Analysis metadata ------------
    geometry_metadata: GeometryMetadata = field(default_factory=GeometryMetadata)                            # Geometry-related metadata
    aerodynamic_metadata: list[AerodynamicMetadata] = field(default_factory=list)  # List of aerodynamic metadata entries, one per Reynolds number or test condition

    # Dict: dictionary of additional arbitrary metadata fields not covered by the above structured attributes
...
def create_new_metadata() -> FoilMetadata:
    """Create a new FoilMetadata object with default values."""
    return FoilMetadata(
        file_name="",
        path="",
        family_series=None,
        source="uiuc_database",
        quality_flags=[],
        is_modified=False,
        is_smooth=False,
        is_naca=False,
...
# For example, to find an AerodynamicMetadata entry by Reynolds number:
def get_aero_by_re(meta, re_target: float, tol: float = 1e-9):
    for aero in meta.aerodynamic_metadata:
        re = aero.reynolds_number
        if re is not None and abs(re - re_target) <= tol:
            return aero
    return None
...
def save_metadata_json(items: list[FoilMetadata], output_path: Path | str, *, ensure_ascii: bool = False, indent: int = 2, ) -> None:
    """Persist a metadata list to JSON."""
...
def load_metadata_json(input_path: Path | str) -> list[FoilMetadata]:
    """Load metadata list from JSON."""
    input_file = Path(input_path)
    with input_file.open("r", encoding="utf-8") as f:
        raw = json.load(f)
    if not isinstance(raw, list):
        raise ValueError("Metadata JSON root must be a list.")
    return [FoilMetadata.from_dict(item) for item in raw]
